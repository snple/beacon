...
// RequestContext 请求上下文（类似 HTTP Request）
type RequestContext struct {
	Request   *core.Request     // 原始请求
...
// Respond 响应请求
func (ctx *RequestContext) Respond(result *core.Response) error {
	if ctx.responded {
...
func (ctx *RequestContext) RespondSuccess(payload []byte) error {
	return ctx.Respond(core.NewResponse(payload))
}

// RespondError 响应错误
func (ctx *RequestContext) RespondError(code uint8, reason string) error {
	return ctx.Respond(core.NewErrorResponse(packet.ReasonCode(code), reason))
}
...
// MessageMiddlewareFunc 中间件函数类型（消息）
type MessageMiddlewareFunc func(ctx *MessageContext, next func())

// RequestMiddlewareFunc 中间件函数类型（请求）
type RequestMiddlewareFunc func(ctx *RequestContext, next func())
...
type Router struct {
	...
	messageMiddlewares []MessageMiddlewareFunc
...
// NewRouter 创建新的路由器
func NewRouter(core *core.Core, opts *RouterOptions) *Router {
	if opts == nil {
...
//	})
func (r *Router) UseMessage(middleware MessageMiddlewareFunc) {
	r.messageMu.Lock()
	defer r.messageMu.Unlock()
...
//	    next()
//	})
func (r *Router) UseRequest(middleware RequestMiddlewareFunc) {
	r.requestMu.Lock()
	defer r.requestMu.Unlock()
...
func (r *Router) messageLoop() {
	...
	for {
		...
		msg, err := r.core.PollMessage(r.ctx, r.pollDelay)
		if err != nil {
			if r.ctx.Err() != nil {
				return // 上下文已取消
			}
...
func (r *Router) requestLoop() {
	...
	for {
		...
		req, err := r.core.PollRequest(r.ctx, r.pollDelay)
		if err != nil {
			if r.ctx.Err() != nil {
				return // 上下文已取消
			}
			...
			if errors.Is(err, core.ErrCoreNotRunning) || errors.Is(err, core.ErrCoreShutdown) {
				return // core 停止，优雅退出
			}
...
func (r *Router) dispatchMessage(msg *core.Message) {
	r.messageMu.RLock()
	handlers := make([]*messageRoute, len(r.messageHandlers))
	copy(handlers, r.messageHandlers)
	middlewares := make([]MessageMiddlewareFunc, len(r.messageMiddlewares))
...
func (r *Router) dispatchRequest(req *core.Request) {
	r.requestMu.RLock()
	handlers := make([]*requestRoute, len(r.requestHandlers))
	copy(handlers, r.requestHandlers)
	middlewares := make([]RequestMiddlewareFunc, len(r.requestMiddlewares))
	...
	if matchedRoute == nil {
		...
		ctx.RespondError(0x83, "action not found: "+req.Action) // UnspecifiedError
		return
	}
	...
	if !ctx.responded {
		...
		ctx.RespondError(0x80, "handler did not respond")
	}
}

// runMessageChain 执行消息中间件链
func (r *Router) runMessageChain(ctx *MessageContext, middlewares []MessageMiddlewareFunc, handler RouterMessageHandlerFunc) {
	index := 0
	var next func()
	next = func() {
...
func (r *Router) runRequestChain(ctx *RequestContext, middlewares []RequestMiddlewareFunc, handler RouterRequestHandlerFunc) {
	index := 0
	var next func()
	next = func() {
		if index < len(middlewares) {
