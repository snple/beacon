...
func (cs *CoreService) GetPinValue(pinID string) (nson.Value, time.Time, error) {
	// basic validation
	if pinID == "" {
		return nil, time.Time{}, fmt.Errorf("please supply valid Pin.Id")
	}

	value, updated, err := cs.GetStorage().GetPinValue(pinID)
	if err != nil {
		// 如果未找到值，返回空值而不是错误
...
func (cs *CoreService) setPinValue(value dt.PinValue) error {
	// basic validation
	...
	err := cs.GetStorage().SetPinValue(value)
	if err != nil {
