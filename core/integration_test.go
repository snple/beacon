...
// TestRequest_ClientToCore 测试客户端向 core 发送请求
func TestRequest_ClientToCore(t *testing.T) {
	core := testSetupCore(t, nil)
	defer core.Stop()

	addr := core.GetAddress()

	c := testSetupClient(t, addr, "req-client", nil)
	defer c.Close()
	...
	go func() {
		...
		resp, reqErr = c.RequestToCore(ctx, "echo", []byte("ping"), nil)
	}()
	...
	req, err := core.PollRequest(ctx, 3*time.Second)
	if err != nil {
		t.Fatalf("Core failed to poll request: %v", err)
	}

	if req.Action != "echo" {
		t.Errorf("Expected action 'echo', got '%s'", req.Action)
	}

	// 响应
	err = req.RespondSuccess([]byte("pong"))
	if err != nil {
	...
	if reqErr != nil {
		t.Fatalf("Client request failed: %v", reqErr)
	}
	...
	if !resp.IsSuccess() {
		t.Errorf("Response not success: %s", resp.Error())
	}
...
// TestRequest_CoreToClient 测试 core 向客户端发送请求
func TestRequest_CoreToClient(t *testing.T) {
	core := testSetupCore(t, nil)
	defer core.Stop()

	addr := core.GetAddress()

	c := testSetupClient(t, addr, "worker-client", nil)
	defer c.Close()

	// 客户端注册 action
	err := c.Register("process")
	if err != nil {
	...
	go func() {
		...
		req, err := c.PollRequest(ctx, 4*time.Second)
		if err != nil {
			t.Errorf("Client failed to poll: %v", err)
			return
		}

		if req.Action != "process" {
			t.Errorf("Expected action 'process', got '%s'", req.Action)
		}

		req.RespondSuccess([]byte("processed: " + string(req.Payload)))
	}()
	...
	resp, err := core.Request(ctx, "process", []byte("data"), nil)
	if err != nil {
	...
	if string(resp.Payload) != "processed: data" {
		t.Errorf("Expected 'processed: data', got '%s'", string(resp.Payload))
	}
}
...
func TestRequest_Timeout(t *testing.T) {
	...
	c := testSetupClient(t, addr, "timeout-client", nil)
	defer c.Close()

	// 注册 action 但不响应（客户端会自动返回超时错误）
	err := c.Register("slow")
	if err != nil {
		t.Fatalf("Failed to register: %v", err)
	}
	...
	// core 发送请求，客户端会返回超时错误响应
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	opts := DefaultRequestOptions().WithTimeout(1 * time.Second)
	resp, err := core.Request(ctx, "slow", []byte("data"), opts)
	// 客户端会自动返回超时错误响应，所以这里会收到响应但带有错误码
	if err == nil && resp != nil && resp.ReasonCode == packet.ReasonSuccess {
		t.Error("Expected timeout or error response")
	}
	// 如果有错误或响应码不是成功，测试通过
}
...
func TestRequest_ErrorResponse(t *testing.T) {
	...
	c := testSetupClient(t, addr, "error-client", nil)
	defer c.Close()
	...
	go func() {
		...
		req.RespondError(packet.ReasonServerBusy, "too busy")
	}()
	...
	resp, err := core.Request(ctx, "fail", []byte("data"), nil)
	if err != nil {
	...
	if resp.ReasonString != "too busy" {
		t.Errorf("Expected 'too busy', got '%s'", resp.ReasonString)
	}
}

// TestRequest_WithProperties 测试带属性的请求响应
func TestRequest_WithProperties(t *testing.T) {
	...
	c := testSetupClient(t, addr, "props-client", nil)
	defer c.Close()

	err := c.Register("props")
	if err != nil {
		t.Fatalf("Failed to register: %v", err)
	}
	...
	go func() {
		...
		if req.UserProperties["custom"] != "value" {
			t.Errorf("Expected UserProperties['custom']='value', got '%s'", req.UserProperties["custom"])
		}

		req.Response(client.NewResponse([]byte("ok")).
	...
	opts := DefaultRequestOptions().
		WithTraceID("trace-123").
		WithContentType("application/json").
		WithUserProperty("custom", "value")

	resp, err := core.Request(ctx, "props", []byte("data"), opts)
	if err != nil {
		t.Fatalf("Request failed: %v", err)
	}

	if resp.TraceID != "trace-123" {
		t.Errorf("Response traceID mismatch: %s", resp.TraceID)
	}
}
